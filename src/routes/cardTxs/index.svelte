<script>
    ...
    const printConversation = (active) => {
      ...
      console.log('printing conversation', active);
        ...
        cardTx = $messages.filter(x => x.chat === chat)
        scrollDown()
    }
    //Chat to add
...
<main in:fade>
    ...
    <div class="board_window" bind:this={box}>
        <ChatWindow>
            {#each cardTx as message}
                <ChatBubble handleType={message.sent} message={message.amount} ownMsg={message.sent} msgFrom={message.user} timestamp={message.t}/>
            {/each}
        </ChatWindow>
      </div>
